...
@section('titulo', 'Todas las motos del concesionario {{ $concesionario->name }} en Larabikes')
...
@section('contenido')
    ...
    @if ($total >= 1)
        <div class="flex flex-row justify-center p-2 m-2">
            <p>
                Hay {{ $total }} Motos en el Concesionario <span class="font-bold underline text-large">{{ $concesionario->name }}</span>
            </p>
        </div>
    @endif
